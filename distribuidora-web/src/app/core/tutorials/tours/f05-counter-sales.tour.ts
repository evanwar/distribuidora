...
export const F05_TOURS: readonly TutorialDefinition[] = [
  {
    ...
    title: copy('Preparar una venta', 'Prepare a sale'), description: copy('Selecciona cliente, encuentra productos y revisa el carrito.'),
    steps: [
      step('header', S.posHeader, copy('Punto de venta', 'Point of sale'), copy('Esta pantalla concentra la venta. El tutorial no agregará productos ni guardará el borrador.')),
      step('customer', S.posCustomer, copy('Cliente y condición', 'Customer and terms'), copy('Busca por nombre o celular. El crédito requiere un cliente válido y autorización del servidor.')),
      step('search', S.posProductSearch, copy('Busca productos', 'Find products'), copy('Usa SKU, código de barras o nombre. Un código exacto permite una selección rápida.')),
      step('results', S.posProductResults, copy('Disponibilidad', 'Availability'), copy('Revisa precio y existencia antes de agregar. Los resultados se actualizan sin descargar todo el catálogo.'), { optional: true }),
      ...
      step('totals', S.posTotals, copy('Totales preliminares', 'Preview totals'), copy('El resumen local ayuda a revisar; la venta confirmada por el servidor es la fuente definitiva.'), { placement: 'left' }),
    ],
  },
  {
    ...
    title: copy('Cómo cobrar con tarjeta', 'How to take a card payment'),
    description: copy('Proceso completo: validar total, elegir tarjeta y terminal, iniciar el cobro y confirmar su resultado.', 'Complete process: verify the total, choose card and terminal, start payment, and confirm its result.'),
    steps: [
  ...
  },
  {
    ...
    title: copy('Cobrar una venta', 'Take payment'), description: copy('Conoce formas de pago, efectivo, cambio y confirmación segura.'),
    steps: [
      ...
      step('method', S.posPaymentMethod, copy('Forma de pago', 'Payment method'), copy('Elige efectivo, tarjeta u otra opción disponible. Crédito y pago mixto dependen del contrato vigente.'), { placement: 'left' }),
      ...
      step('change', S.posChange, copy('Cambio a devolver', 'Change due'), copy('Entrega únicamente el cambio mostrado. Nunca se presenta un importe negativo.'), { placement: 'left', optional: true }),
