...
export const F02_TOURS: readonly TutorialDefinition[] = [
  {
    ...
    title: copy('Cómo registrar un cliente', 'How to register a customer'), description: copy('Proceso completo: buscar duplicados, capturar los datos, validar y confirmar el alta.', 'Complete process: check for duplicates, enter the details, validate them, and confirm the new customer.'),
    steps: [
      ...
      step('search', S.customersSearch, copy('1. Descarta duplicados', '1. Check for duplicates'), copy('Antes de crear, busca por nombre, RFC, teléfono y correo. Si ya existe, abre ese registro en vez de generar otro.', 'Before creating one, search by name, tax ID, phone, and email. If it exists, open that record instead of creating a duplicate.')),
      ...
      step('new', S.customersPrimaryAction, copy('3. Inicia el alta', '3. Start registration'), copy('Pulsa “Nuevo cliente”. Se abrirá la captura; este tutorial no presionará el botón ni enviará el formulario.', 'Choose “New customer” to open the form. This tour will not press the button or submit the form.')),
      step('identity', S.customersEditor, copy('4. Captura la identidad', '4. Enter identity details'), copy('Registra el nombre o razón social y el RFC cuando aplique. Usa datos reales y evita abreviaturas que dificulten búsquedas futuras.', 'Enter the name or legal business name and tax ID when applicable. Use accurate data and avoid abbreviations that make future searches harder.'), { optional: true }),
  ...
  },
  {
    ...
    requiredPermissions: ['catalogs.view', 'catalogs.create', 'catalogs.edit'],
    title: copy('Cómo registrar un producto', 'How to register a product'), description: copy('Proceso completo: evitar duplicados, capturar SKU, clasificar, definir precios y validar el alta.', 'Complete process: avoid duplicates, enter the SKU, classify the item, set prices, and validate the result.'),
    steps: [
      ...
      step('identity', S.entityEditor, copy('3. Identifica el producto', '3. Identify the product'), copy('Asigna un SKU único, nombre claro y código de barras correcto. Escanea o compara el código para evitar errores de captura.', 'Assign a unique SKU, a clear name, and the correct barcode. Scan or compare the code to avoid entry errors.'), { optional: true }),
      step('classification', S.entityEditor, copy('4. Clasifica y define la unidad', '4. Classify and set the unit'), copy('Selecciona categoría, marca y unidad de medida. Estas opciones afectan búsquedas, reportes y cómo se recibe o vende el artículo.', 'Select the category, brand, and unit of measure. These choices affect search, reports, and how the item is received or sold.'), { optional: true }),
